...
class UrbanSound8kBrain(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        self.acc_metric.append(predictions, classid, lens)
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            self.train_stats = {
                ...
                "acc": self.acc_metric.summarize(),  # "acc": self.train_acc_metric.summarize(),
            }
        # Summarize Valid statistics from the stage for record-keeping.
        elif stage == sb.Stage.VALID:
            valid_stats = {
                "loss": stage_loss,
                "acc": self.acc_metric.summarize(),  # "acc": self.valid_acc_metric.summarize(),
            }
        # Summarize Test statistics from the stage for record-keeping.
        else:
            test_stats = {
                "loss": stage_loss,
                "acc": self.acc_metric.summarize(),  # "acc": self.test_acc_metric.summarize(),
            }
...
def dataio_prep(hparams):
    ...
    for dataset in ["train", "valid", "test"]:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_csv(
            csv_path=os.path.join(hparams['save_folder'], '{}_task0_raw.csv'.format(dataset)),
