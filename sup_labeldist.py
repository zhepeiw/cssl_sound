...
class SupCLR(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        if self.hparams.prev_embedding_model is not None:
            ...
            self.modules.prev_classifier.eval()
            if batch.dist_sig1[0] is not None:
                ...
                o1_prev = self.modules.prev_classifier(h1_prev)
            else:
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # distillation loss
        if self.hparams.prev_embedding_model is not None:
            o1_prev = predictions['o1_prev']
            dist_loss = self.hparams.compute_dist_cost(o1, o1_prev)
        else:
...
if __name__ == "__main__":
    ...
    for task_idx in range(start_task, num_tasks):
        ...
        if not hparams['resume_interrupt']:
            ...
            if task_idx > 0:
                hparams['recoverables']['prev_classifier'] = \
                        hparams['modules']['prev_classifier'] = \
                        hparams['prev_classifier'] = hparams['prev_classifier_fn']()
                ...
                prev_embedding_checkpointer = sb.utils.checkpoints.Checkpointer(
                    ...
                    recoverables={
                        ...
                        'classifier': hparams['prev_classifier']
                    },
                )
                ...
                for p in hparams['prev_classifier'].parameters():
                    p.requires_grad = False
            else:
                hparams['prev_classifier'] = None
