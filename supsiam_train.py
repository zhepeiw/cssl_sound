...
class SimSiam(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        z1 = self.modules.projector(h1)  # [B, 1, D]
        z2 = self.modules.projector(h2)  # [B, 1, D]
        p1 = self.modules.predictor(z1)  # [B, 1, D]
        p2 = self.modules.predictor(z2)  # [B, 1, D]
        o1 = self.modules.classifier(h1) # [B, 1, C]
        #  o2 = self.modules.classifier(h2) # [B, 1, C]

        return p1, p2, z1.detach(), z2.detach(), o1, lens
    ...
    def compute_objectives(self, predictions, batch, stage):
        p1, p2, z1, z2, o1, lens = predictions
        # SSL loss
        ssl_loss_1 = -self.hparams.compute_simsiam_cost(p1, z2).mean()
        ssl_loss_2 = -self.hparams.compute_simsiam_cost(p2, z1).mean()
        ssl_loss = 0.5 * (ssl_loss_1 + ssl_loss_2)
        # supervised loss
